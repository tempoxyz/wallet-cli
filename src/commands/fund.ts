...
async function fundingBalance(options: {
  ...
  const provider = createProvider({ network: options.chainId === 42431 ? 'testnet' : undefined }) as
    CoreProvider.Provider & { getClient: () => unknown }
...
function fundUrl(action: FundAction, options: { code?: string | undefined } = {}) {
  const url = new URL('https://wallet.tempo.xyz/')
