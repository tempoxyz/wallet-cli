...
export async function fetchServices(options: {
  ...
  if (options.search) return summaries.filter((service) => matchesSearch(service, options.search ?? ''))
  return summaries
}
...
async function fetchServiceEntries(): Promise<{ summary: ServiceSummary; detail: ServiceDetail }[]> {
  ...
  const services = getArray(body.services).map((service) => {
    ...
    return {
      ...
      detail: {
        ...
        ...(Array.isArray(item.endpoints) ? { endpoints: item.endpoints.map(serviceEndpoint) } : {}),
      },
    }
  })
  return services
...
function supportsCredits(serviceUrl: string) {
  return /\.mpp\.tempo\.xyz$/.test(new URL(serviceUrl).hostname)
}
