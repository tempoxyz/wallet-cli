...
export async function listSessions(options: {
  ...
  const records = options.all || options.orphaned
...
export async function dryRunCloseSessions(options: {
  ...
  throw usageError('Specify a URL, channel ID (0x...), or use --all/--orphaned/--finalize to close sessions')
}
...
export async function closeSessions(options: {
  ...
  if (options.finalize) {
    for (const target of targets.filter((target) => closeTargetState(target) === 'finalizable'))
      await recordCloseResult(summary, closeOneSession(target, { finalize: true, network: options.network }), target)
    return summary
  }
...
async function closeTargets(options: {
  ...
  const chain = chainId(options.network)
  const network = networkName(chain) ?? 'tempo'
  const records = options.all || options.orphaned || options.finalize
    ? await discoverAndPersistOrphanedChannels({ network: options.network })
    : await readChannelRecords()
  const local = records.filter((record) => record.network === network)
...
async function closeOneSession(
  ...
  options: { cooperative?: boolean | undefined; finalize?: boolean | undefined; network?: string | undefined },
): Promise<CloseResultRecord> {
  if (!isChannelId(record.channel_id))
    return { channel_id: record.channel_id, status: 'error', origin: record.origin, error: 'no active session' }

  const chain = chainId(options.network)
  const escrow = (record.escrow_contract || escrowContract(chain)) as Address
  const channelId = record.channel_id as Hex
  const onChain = await getOnChainChannel({ channelId, escrowContract: escrow, network: options.network })
  if (!onChain) {
    await deleteChannelRecord(record.channel_id)
...
async function closeSessionCooperatively(options: {
  ...
  const chain = chainId(options.network)
  const challenge = await refreshSessionChallenge(closeUrl) ?? storedSessionChallenge(options.record)
  if (!challenge) throw new Error('cooperative close requires a stored or fresh session payment challenge')

  const cumulativeAmount = options.record.accepted_cumulative > 0n
...
function storedSessionChallenge(record: ChannelRecord) {
  try {
    ...
    const request = typeof value.request === 'string'
...
async function sendSessionManagementTransaction(options: {
  ...
  if (!txHash) throw new Error(`${options.functionName} submitted but receipt did not include a transaction hash`)
  return txHash
}
...
async function recordCloseResult(
  ...
  try {
    ...
    summary.failed += 1
    summary.results.push(closeResult(target, 'error', {
...
function sessionItem(record: ChannelRecord) {
  const spent = record.accepted_cumulative > 0n ? record.accepted_cumulative : record.cumulative_amount
  ...
  return {
    ...
    ...(status === 'closing' || status === 'finalizable'
      ? { remaining_secs: record.grace_ready_at > nowSeconds() ? record.grace_ready_at - nowSeconds() : 0 }
      : {}),
...
async function readChannelRecords(): Promise<ChannelRecord[]> {
  ...
  try {
    ...
    if (message.includes('unable to open database file') || message.includes('no such table')) return []
    throw error
  }
...
async function readGracePeriod(options: {
  escrowContract: Address
  network?: string | undefined
}) {
  try {
...
async function saveDiscoveredChannel(options: {
  ...
  const now = nowSeconds()
  const graceReadyAt = options.channel.closeRequestedAt === 0n
    ? 0
...
function sessionStatus(record: ChannelRecord) {
  if (record.state === 'closing') {
    return record.grace_ready_at > 0 && record.grace_ready_at <= nowSeconds() ? 'finalizable' : 'closing'
  }
  return record.state
}

function channelState(value: string): ChannelState {
  if (value === 'closing' || value === 'finalizable' || value === 'finalized' || value === 'orphaned')
    return value
  return 'active'
}
