...
export async function transferTokens(options: {
  ...
  const { args } = options
  if (!args.amount || !args.token || !args.to) throw new Error('amount, token, and to are required')
  ...
  if (!from) throw usageError("Configuration missing: No wallet configured. Run 'tempo wallet login'.")
...
export async function transferCredits(options: {
  ...
  if (!activeAccount)
    throw usageError("Configuration missing: No wallet configured. Run 'tempo wallet login'.")
  if (!wallet) throw usageError("Configuration missing: No wallet configured. Run 'tempo wallet login'.")
  ...
  if (!txHash) throw new Error('Credits redeem submitted but response did not include a transaction hash')
...
function buildMppCreditsTransfer(options: {
  ...
  if (challenge.method !== 'tempo')
    throw usageError(`Invalid configuration: unsupported MPP method for Coinflow credits: ${challenge.method}`)
  if (challenge.intent !== 'charge')
    throw usageError(`Invalid configuration: unsupported MPP intent for Coinflow credits: ${challenge.intent}`)
  ...
  if (token !== usdcToken) throw usageError(`Invalid configuration: MPP challenge currency ${token} is not ${usdcToken}`)
  if (!/^0x[0-9a-f]{40}$/.test(recipient))
    throw usageError('Invalid configuration: MPP challenge is missing a recipient address')
  if (!/^\d+$/.test(amount)) throw usageError(`Invalid configuration: invalid MPP amount: ${amount}`)
  ...
  const data = encodeFunctionData({
    ...
    args: [recipient as `0x${string}`, atomicAmount, mppAttributionMemo(challenge, options.clientId)],
  })
...
function buildCreditsTransactionData(options: { to: string; data: string; value: string }) {
  if (!isZeroValue(options.value))
    throw usageError('Invalid configuration: Coinflow credits redeem does not support non-zero ETH value')
...
function parseMppChallenge(input: string) {
  ...
  if (paymentIndex < 0) throw usageError("Invalid configuration: invalid MPP challenge: Expected 'Payment' scheme.")
  ...
  if (!request) throw usageError('Invalid configuration: invalid MPP challenge: Missing request parameter.')
...
function parseAuthParams(input: string) {
  ...
  while (index < input.length) {
    ...
    index = nextIndex
    if (key in result)
      throw usageError(`Invalid configuration: invalid MPP challenge: Duplicate parameter: ${key}.`)
    result[key] = value
  }
...
function amountToUsdCents(amountAtomic: bigint) {
  ...
  if (cents > BigInt(Number.MAX_SAFE_INTEGER))
    throw usageError(`Invalid configuration: MPP challenge amount ${amountAtomic.toString()} is too large for Coinflow credits`)
  return Number(cents)
}
...
function hashPrefix(value: string, length: number) {
  return Buffer.from(keccak256(new TextEncoder().encode(value)).slice(2), 'hex').subarray(0, length)
}
